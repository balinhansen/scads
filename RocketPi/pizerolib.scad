...
module ScrewHole(){
    translate([0,0,-0.002])
    cylinder(thickness+0.004,screw_width/2,screw_width/2,$fn=fineness);
}



// IO Cutouts, Positions and Extrusions
...
function getBoardThickness()=board_thickness;

module PiZeroHolePositions(){
    translate([3.5,3.5,0])
    children();
...
module PiZeroBoardShape(){
    hull(){
        translate([board_bevel,board_bevel,0])
        children();
        
        translate([board_bevel,board_width-board_bevel,0])
        children();
        
        translate([board_length-board_bevel,board_bevel,0])
        children();
        
        translate([board_length-board_bevel,board_width-board_bevel,0])
        children();
    }
...
module PiZeroBoard(){
    difference(){
        PiZeroBoardShape()
        cylinder(board_thickness,board_bevel,board_bevel,$fn=fineness);
...
module m25_peg(length){
    translate([0,0,length])
    sphere(2.4/2,$fn=fineness);
    translate([0,0,0])
    cylinder(length,2.4/2,2.4/2,$fn=fineness);
}
